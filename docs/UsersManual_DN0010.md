...
| 基板Mid内側 | 2 | 同一形状の2枚を、左用、右用として使用。 |
| 基板Mid外側 | 2 | 同一形状の2枚を、左用、右用として使用。 |
| 基板Top | 2 | 同一形状の2枚を、左用、右用として使用。 |
| Base用スイッチプレート | 2 | 同一形状の2枚を、左用、右用として使用。 |
| Mid内側用スイッチプレート | 2 | 同一形状の2枚を、左用、右用として使用。|
| Mid外側用スイッチプレート | 2 | 同一形状の2枚を、左用、右用として使用。 |
| Top用スイッチプレート | 2 | 同一形状の2枚を、左用、右用として使用。 |
| ボトムプレートA | 2 | 〓未定 |
...
| 短いプラネジ<br>M2 3mm | 12 | []() |
| ゴム足（小） | 4 | 〓半球っぽいやつにしたい |
| USBコネクタキャップ<br>〓 | 1 | 出荷時は`基板Base（みぎ）`へ取り付け済み。 |
| ダイオード<br>1N4148W SOD123 | 15 | 予備3個含む。<br>[高速スイッチング・ダイオード 1N4148W](https://akizukidenshi.com/catalog/g/g107084/) |
| LED<br>WS2812C-2020 | 6 | []() |
...
| ゴムシート<br>T=1mm 10mm * 500mm | 〓未定 | [低弾性ゴムシート－ハネナイトＲ－](https://jp.misumi-ec.com/vona2/detail/110300277050/?ProductCode=UNLEA1-10) |
| ポロンシート<br>T=1.5mm 50mm * 〓mm | 1 | [イノアック L32-1.550-MT マイクロセルウレタンロールPORON 黒 1.5×50mm×1M巻](https://www.yodobashi.com/product/100000001004002516/) |

※手に入れやすい製品を記載していますが、出荷時期によって、機能的に同等なものへ変更することがあります。
...
| USBケーブル （Type-C to Type-C） | 1 | 左右の通信用に使用します。 |
...
1. `Chocスイッチ用ソケット`を取り付ける。（6個）
    ...
    ![ダイオードの方向](../assets/UsersManual_DN0010/icon_socket_direction.png)
...
1. 【該当者のみ】うら側から`スピーカー`を取り付ける。
    ...
    ![LED取り付けマーキング](../assets/UsersManual_DN0010/pcb_base_back_speaker.png)
